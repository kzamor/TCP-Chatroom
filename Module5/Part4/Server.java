...
public class Server {
	...
	private Room getRoom(String roomName) {
		for (int i = 0, l = rooms.size(); i < l; i++) {
			if (rooms.get(i).getRoomName().equalsIgnoreCase(roomName)) {
				return rooms.get(i);
			}
		}
		return null;
	...
	protected synchronized boolean joinRoom(String roomName, ServerThread client) {
		...
		if (newRoom != null) {
			if (oldRoom != null) {
				System.out.println(client.getName() + " leaving room " + oldRoom.getRoomName());
				oldRoom.removeClient(client);
			}
			System.out.println(client.getName() + " joining room " + newRoom.getRoomName());
			newRoom.addClient(client);
			return true;
		}
        else{
            client.send(String.format("Room %s wasn't found, please try another", roomName));
        }
		return false;
	}
    /***
    ...
    protected synchronized void removeRoom(Room r){
        if(rooms.removeIf(room->room==r)){
            System.out.println("Removed empty room " + r.getRoomName());
        }
    }
